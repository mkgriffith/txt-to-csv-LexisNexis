...
sThe original code (split_ln.py) was created by Neal Caren on 2012-05-14 
[neal.caren@unc.edu]
...
def split_ln(fname):
    ...
    lnraw = re.sub(r'\r\n', '\n', lnraw) #Normalize line endings.

    

    workfile=re.sub(r'                Copyright .*?\n','ENDOFILE',lnraw) #silly hack to find the end of the documents

    workfile1=workfile.replace('\xef\xbb\xbf\n','') #clean up crud at the beginning of the file

    workfile2=workfile1.split('ENDOFILE') #split the file into a list of documents.

    workfile3=[f for f in workfile2 if len(f.split('\n\n'))>2] #remove an blank rows
    ...
    meta_list=[m for m in meta_list if float(lnraw.count(m))/len(workfile3)>.20] #Keep only the commonly occuring ones
